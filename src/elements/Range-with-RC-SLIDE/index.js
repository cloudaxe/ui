...
const toolTipHandleFunc = props => {
  ...
  return (
    <Tooltip
      prefixCls="cloudxui__rc_slider_tooltip rc-slider-tooltip"
      overlay={value}
      visible={dragging}
      placement="top"
      key={index}
...
export default ({ className, labelText, labelPosition, type, ...props }) => {
  ...
  const addAllClasses = ['cloudxui__rangebox'];
  ...
  const LabelField = labelText && (
    <span className="cloudxui__field-label">{labelText}</span>
  );
